...
public class MainLOKitThread extends Thread {
...
//    private LibreOfficeMainActivity mApplication;
     ...
     */
    private void redraw() {
        MainLOKitShell.sendEvent(new LOEvent(LOEvent.RENDER_TILE));
...
//        mInvalidationHandler = new InvalidationHandler(LibreOfficeMainActivity.mAppContext);
        mTileProvider = MainTileProviderFactory.create(mLayerClient, mInvalidationHandler, filename);
...
//            LOKitShell.hideProgressSpinner();
    ...
    private void loadDocument(String filename, int renderWidth, int renderHeight){
        mTileProvider = MainTileProviderFactory.create(mLayerClient, mInvalidationHandler, filename, renderWidth, renderHeight);
...
//        mTileProvider.rerenderTile();
        if (mTileProvider != null && mTileProvider instanceof MainLOKitTileProvider){
            MainLOKitTileProvider mainLOKitTileProvider = (MainLOKitTileProvider)mTileProvider;
//            mainLOKitTileProvider.reanderTile(1.0f);
            mainLOKitTileProvider.renderTileWidth(1.0f);
        }
    }
    ...
    private void processEvent(LOEvent event) {
        switch (event.mType) {
            ...
            case LOEvent.CLOSE:
                closeDocument();
                break;
            case LOEvent.RENDER_TILE:
                renderTile();
                break;
            case LOEvent.LOAD_SIZE:
                loadDocument(event.mString, event.mRenderWidth, event.mRenderHeight);
                break;
